...
import { CountByCategoryData } from "@/lib/mock-data-lead-value"
      ...
      <div className="bg-white border border-slate-200 p-3 rounded-lg shadow-enterprise-md text-sm">
        ...
        {payload.map((entry: any) => (
          <p key={entry.dataKey} style={{ color: entry.color }} className="font-semibold">
            {entry.name}: {entry.dataKey === "avgLeadValue" ? `${entry.value.toFixed(1)}` : entry.value.toLocaleString()}
          </p>
        ))}
      </div>
    )
...
export function CountByCategoryChart({ data }: Props) {
    ...
    <Card className="rounded-xl border-slate-200 shadow-enterprise-sm flex flex-col h-full">
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-semibold text-slate-800">
          Sum of count by optional_column_value
        </CardTitle>
      </CardHeader>
        ...
        <div className="flex flex-col lg:flex-row gap-5 h-full">
          ...
          <div className="lg:w-[200px] shrink-0">
            <div className="border border-slate-200 rounded-lg p-4 bg-white">
              ...
              <p className="text-xs text-slate-500 mb-2">optional_colum...</p>
            ...
            <ResponsiveContainer width="100%" height={380}>
              ...
              >
                ...
                <Line
                  yAxisId="right"
                  type="monotone"
                  dataKey="avgLeadValue"
                  name="Average lead value (R)"
                  stroke="#22c55e"
                  strokeWidth={2}
                  dot={{ r: 3, fill: "#22c55e" }}
